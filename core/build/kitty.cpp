...
int calculate(int x1, int y1 , int x2 , int y2)
{
	...
	//cout << "result" << result*1.1111 << endl;
...
void findids()
{
	...
	for (int i = 1; i < contours[cont_index].size(); i++)
	{
		...
		//cout << measruement1.x1 << "kkk" << measruement1.y1 << "index" << contours[cont_index][i] << "   "<<i<<endl;
		...
		{
		//	cout << contours[cont_index][i] << endl;
...

{
	...
	if (id1 > id2)
	{
		...
		//	cout << "hi       " << i <<"   "<< contours[cont_index][i] << endl;
			...
			{
				//count_pix = count_pix + calculate(xx1, yy1, xx2, yy2);
			...
			else if ((xx1 - xx2) != 0 && (yy1 - yy2) != 0)
			{
				...
				//count_pix = count_pix + (1.414*calculate(xx1, yy1, xx2, yy2));
				len1 = len1 + (1.340*calculate(xx1, yy1, xx2, yy2));


			}
		//	cout << "length a" << i << " " << len1*pixlen <<endl;
	...
	if (id1 < id2)
	{
		...
		//	cout << "length b" << i << " " << len1*pixlen << endl;
			if (set == 0 && midlen >= len1*pixlen) {
				mid1x = xx1;
				mid1y = xx2;
			}
	...
	if (len1 > 3000){
		...
		if (id1 < id2)
		{
			...
			for (int i = id1; 2< i; i--)
			{
				...
				//len3 = len3 + calculate(xx1, yy1, xx2, yy2);
...
{
	//int tot=0;
	//int totid = contours[cont_index].size();    //contours[cont_index].size();
	//
	//
	//if (topmid1<topmid2 && id2>totid - 200)
	//{
	//

	//	int midp=(totid - (topmid2 - topmid1))/2;
	//	if (totid - topmid2 > midp)
	//	{
	...
	//if (topmid1>topmid2 && id2>0)
	//{


	//	midid1 =topmid1-(topmid1 - topmid2)/2;
